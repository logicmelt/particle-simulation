from geant4_pybind import G4VUserDetectorConstruction, G4NistManager, G4State, G4Material, G4VUserDetectorConstruction, G4Box, G4Sphere, G4LogicalVolume, G4PVPlacement, G4ThreeVector
# Import units
...
class DetectorConstruction(G4VUserDetectorConstruction):
    def __init__(self, config: dict):
        ...
        self.density_points = config["atmos_n_points"]
        self.atmosphere_height = config["atmos_height"] * km # Height of the atmosphere
        self.size = config["atmos_size"] * km # Size of the world volume. Arc lenght for the sphere, side for the box
        self.shape = config["geometry"] # Shape of the world volume
    ...
    def construct_flat_world(self):
        ...
        for i in range(self.density_points):
            ...
            logicAtmos[i] = G4LogicalVolume(solidAtmos[i], 
                                            ...
                                            "logicAtmos_" + str(i))
            G4PVPlacement(None, 
                          G4ThreeVector(0, 0, i * self.atmosphere_height / self.density_points + box_height / 2 - self.atmosphere_height / 2), 
                          logicAtmos[i], 
                          "physAtmos_" + str(i), 
                          logicWorld, 
                          False, 
    ...
    def construct_spherical_world(self):
        # Earth radius
        earth_rad = self.config["earth_radius"]
        # Get the angle to create the spherical sector
        angle_sph = self.size / earth_rad * radian

        # Create the world volume
        solidWorld = G4Sphere("solidWorld", earth_rad, earth_rad + self.atmosphere_height, 0, angle_sph, 0, 2 * np.pi * radian)
        logicWorld = G4LogicalVolume(solidWorld, self.world_material, "logicWorld")
    ...
    def parse_density_temp_from_config(self):
        ...
        if self.config["atmos_density"] == "from_file":
            # Read the density and temperature profile from a file
            file_path = self.config["atmos_from_file"]
